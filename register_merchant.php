...
if (isset($_POST['sign_up']) && $_POST['sign_up'] == "0") {
    ...
    if ($countEmail > 0) {
        ...
        $logoName = null;
        $uploadDir = $_SERVER['DOCUMENT_ROOT'] . "/palio_browser/images/";
        ...
        if (isset($_POST['is_artist']) && $_POST['is_artist'] == "1") {
            $query = $dbconn2->prepare(
                "INSERT INTO SHOP(`CODE`, `NAME`, `CREATED_DATE`, `DESCRIPTION`, `FILE_TYPE`, `THUMB_ID`, `LINK`, `CATEGORY`, `USE_ADBLOCK`, `PALIO_ID`, `CREATED_BY`) VALUES (?, ?, ?, '-', 1, ?, ?, '4', 0, ?, ?)");
            $query->bind_param("sssssss", $hex, $company_name, $sent_time, $logoName, $link, $store_palio_id, $created_by);
            $status = $query->execute();
            $query->close();
        } else {
            $query = $dbconn2->prepare(
                "INSERT INTO SHOP(`CODE`, `NAME`, `CREATED_DATE`, `DESCRIPTION`, `FILE_TYPE`, `THUMB_ID`, `LINK`, `CATEGORY`, `USE_ADBLOCK`, `PALIO_ID`, `CREATED_BY`) VALUES (?, ?, ?, '-', 1, ?, ?, '0', 0, ?, ?)");
            $query->bind_param("sssssss", $hex, $company_name, $sent_time, $logoName, $link, $store_palio_id, $created_by);
            $status = $query->execute();
            $query->close();
        }
