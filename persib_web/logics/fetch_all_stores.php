...
if(isset($store_id)){
    $query = $dbconn->prepare("SELECT s.*, be.ID as BE_ID, be.EMAIL_SUPPORT as BE_EMAIL FROM SHOP s left join BUSINESS_ENTITY be on s.PALIO_ID = be.COMPANY_ID WHERE s.CODE = ? order by s.SCORE desc");
