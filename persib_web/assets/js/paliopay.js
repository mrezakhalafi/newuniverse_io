...
function fetchRewardPoints() {
    ...
    xmlHttp.open("GET", "/persib_web/logics/fetch_stores_reward_user_raw.php?f_pin=" + fpin_lokal);
    xmlHttp.send();
}
fetchRewardPoints();
...
function getItemDetail(item_name){
    ...
    // xhr.open("POST", "/persib_web/logics/fetch_item_details.php", false);
    // xhr.send(formData);
...
function getMerchantAddress(merchant_name){
    ...
    // xhr.open("POST", "/persib_web/logics/fetch_merchant_address.php", false);
    // xhr.send(formData);
    ...
    if (localStorage.getItem("shipping-form") == undefined) {
        ...
        for (let i = 0; i < merchantCount; i++) {
            ...
            for (let item of items) {
                ...
                xhr.open("POST", "/persib_web/logics/shipment_api/paxel_shipments_rate");
                xhr.send(formData);
...
function trackShipment(airway_bill){
    ...
    xhr.open("POST", "/persib_web/logics/shipment_api/paxel_track_shipment_status", false);
    xhr.send(formData);
...
function checkEwallet(id) {
    ...
    xhr.open('GET', 'http://202.158.33.26/persib_web/logics/ewallet_check?id=' + id);
    ...
    xhr.onload = async function () {
        if (xhr.status != 200) { // analyze HTTP status of the response
            ...
            if (responseObj.status == "SUCCEEDED") {
                ...
                // postForm("http://192.168.0.56/persib_web/logics/store_payment", { fpin: fpin, method: responseObj.channel_code, status: "success", items: items });
...
function storeAWB(awb_code, shipping_cost, est_pickup, est_arrival, created_datetime, merchant_name){
    ...
    // 2. Configure it: GET-request for the URL /article/.../load
    xhr.open('POST', '/persib_web/logics/shipment_api/paxel_store_awb');
...
function getOrders() {
    ...
    xhr.open("POST", "/persib_web/logics/shipment_api/paxel_get_orders", false);
    xhr.send(formData);
...
function createShipment(xendit) {
    ...
    for (let merchant of merchants) {
        ...
        // 2. Configure it: GET-request for the URL /article/.../load
        xhr.open('POST', '/persib_web/logics/shipment_api/paxel_create_shipments');
...
function fetchAllStores() {
    let xhr = new XMLHttpRequest();
    xhr.open('GET', '/persib_web/logics/fetch_all_stores.php');
    xhr.responseType = 'json';
...
function fetchProducts() {
    ...
    console.log(params);
    xhr.open('GET', '/persib_web/logics/fetch_products_json.php' + params);
    xhr.responseType = 'json';
...
function updateRewardPts() {
    ...
    xhr.open('POST', '/persib_web/logics/update_store_rewards');
    xhr.responseType = 'json';
...
function toSubmitOVO() {
    ...
    $.post("http://202.158.33.26/paliobutton/php/paliopay_ovo",
        ...
        function (data, status) {
            try {
                if (data == "SUCCEEDED") {
                    // postForm("http://192.168.0.56/persib_web/logics/store_payment", { fpin: fpin, method: "ovo", status: "success", items: items });
        ...
        function (data, status) {
            try {
                if (data.status == "CAPTURED") {
                    // postForm("http://192.168.0.56/persib_web/logics/store_payment", { fpin: fpin, method: "card", status: "success", items: items });
...
function getShoppingHistory(fpin) {
    ...
    xmlHttp.open("GET", "/persib_web/logics/fetch_shopping_history.php?f_pin=" + fpin);
    xmlHttp.send();
}
