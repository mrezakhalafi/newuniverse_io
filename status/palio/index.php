...
<?php
    ...
    $query = $dbconn->prepare("SELECT SIGNATURE FROM TRANSACTION_HISTORY WHERE TRANSACTION_ID = ?");
    $query->bind_param("i", $decoded->TransactionID);
    $query->execute();
    $dataHistory = $query->get_result()->fetch_assoc();
    $query->close();

    if($cnt > 0 && $user_account['STATUS'] != 1 && $dataHistory['SIGNATURE'] == $decoded->Signature){
        ...
        $query->bind_param("iii", $bill_id, $user_id, $company_id);
        $query->execute();
        $query->close();
        ...
        $method = 'SAKUKU';
        $content = invoiceMail($name, $orderNumber, $orderDate, $item, 'Rp. '.$price, $method);
