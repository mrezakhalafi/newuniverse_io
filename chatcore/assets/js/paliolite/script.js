...
DOM.chatList.innerHTML = `
	<div class="chat-list-item d-flex flex-row w-100 p-2">
	...
	chatList
		...
		.forEach((elem, index) => {
			...
			if (!elem.isGroup) {
				...
				if (elem.contact.id == localStorage.cc_alias_fpin) { // is company account
				...
				if (elem.is_complain == 1) {
					DOM.chatList.innerHTML += `
					<div id="${elem.contact.id}" class="chat-list-item d-none flex-row w-100 p-2 border-bottom ${unreadClass}" onclick="generateMessageArea(this, ${index}, false, false)">
						...
						<div class="w-50">
							...
							<div class="small last-message">${elem.isGroup ? contactList.find(contact => contact.id === elem.msg.sender).name + ": " : ""}${elem.msg.sender === user.id ? statusClass : ""} <span style="pointer-events:none;">${messagePreview}</span></div>
						</div>
					...
					if (elem.msg.hasOwnProperty('title')) {
						...
						<div class="chat-list-item d-flex flex-row w-100 p-2 border-bottom ${unreadClass}" onclick="generateMessageArea(this, ${index}, false, false)">
							...
							<div class="w-50">
								<div class="name">${badge} ${elem.name}</div>
								<div class="small last-message">${elem.isGroup ? contactList.find(contact => contact.id === elem.msg.sender).name + ": " : ""}${elem.msg.sender === user.id ? statusClass : ""} <span style="pointer-events:none;">${richText(elem.msg.about)} - ${richText(elem.msg.title)}</span></div>
							</div>
							<div class="flex-grow-1 text-right">
								<div class="small time">${mDate(elem.msg.time).chatListFormat()}</div>
								${elem.unread ? "<div class=\"badge badge-success badge-pill small\" id=\"unread-count\">" + elem.unread + "</div>" : ""}
						...
						<div id="accordion-${elem.group.id}" style="width:100%;">
							<div class="card">
								<div class="card-header" id="group-${elem.group.id}">
									<div class="chat-list-item d-flex flex-row w-100 p-2 border-bottom collapsed ${unreadClass}" data-toggle="collapse" data-target="#topic-${elem.group.id}" aria-expanded="false" aria-controls="topic-${elem.group.id}">
										<img src="${elem.isGroup ? elem.group.pic : elem.contact.pic}" alt="Profile Photo" class="img-fluid rounded-circle mr-2" style="height:50px; width:50px;">
									...
									<div class="card-body">
										<div id="${elem.group.id}" class="chat-list-item d-flex flex-row w-100 p-2 border-bottom ${unreadClass}" onclick="generateMessageArea(this, ${index}, false, true)">
											...
											<div class="w-50 align-self-center">
												...
												<div class="small last-message">${elem.isGroup ? (groupMembers.some(el => el.f_pin === elem.msg.sender && el.group_id === elem.msg.recvId) ? (elem.msg.sender == user.id ? "You" : groupMembers.find(el => el.f_pin === elem.msg.sender && el.group_id === elem.msg.recvId).name) : 'member') + ": " : ""}${elem.msg.sender === user.id ? statusClass : ""} <span style="pointer-events:none;">${messagePreview}</span></div>
											</div>
											<div class="flex-grow-1 text-right">
												<div class="small time">${mDate(elem.msg.time).chatListFormat()}</div>
												${elem.unread ? "<div class=\"badge badge-success badge-pill small\" id=\"unread-count\">" + elem.unread + "</div>" : ""}
...
let fwdMessage = (dest) => {
	...
	if (user.isOnline === 1) {
		...
		formData.append('is_chrome', 'true');
		...
		alert('You are currently offline. Please make sure your newuniverse is online.');
	}
};
...
let fillFriendFwdList = () => {
	...
	contactList
		...
		.forEach((elem, index) => {
			...
			if (elem.id !== localStorage.F_PIN && elem.id != '-999') { // exclude self from friend list view, for arr = friend list
				let badge = '';
				if (elem.id == localStorage.cc_alias_fpin) { // is company account
					badge = '<i class="fas fa-check-circle" style="color:#306EFF;"></i>';
	...
	groupList
		...
		.forEach((elem, index) => {
			...
			if (elem.parent === "") {
				...
				<div id="accordion-groupList-fwd-${elem.id}" style="width:100%;">
					<div class="card">
						<div class="card-header" id="groupList-fwd-${elem.id}">
							<div class="chat-list-item d-flex flex-row w-100 p-2 border-bottom collapsed" data-toggle="collapse" data-target="#topic-groupList-fwd-${elem.id}" aria-expanded="false" aria-controls="topic-groupList-${elem.id}">
								<img src="${elem.pic}" alt="Profile Photo" class="img-fluid rounded-circle mr-2" style="height:50px; width:50px;">
								<div class="w-50 align-self-center">
									<div class="name">${elem.name}</div>
								</div>
						...
						<div id="topic-groupList-fwd-${elem.id}" class="collapse" aria-labelledby="groupList-fwd-${elem.id}" data-parent="#accordion-groupList-fwd-${elem.id}">
							<div class="card-body">
								<div class="chat-list-item d-flex flex-row w-100 p-2 border-bottom" onclick="generateMessageArea(this, '${elem.id}', true, true); fwdMessage('${elem.id}')">
									<img src="/chatcore/assets/img/forum.png" alt="Profile Photo" class="img-fluid rounded-circle mr-2" style="height:50px; width:50px;">
									<div class="w-50 align-self-center">
										<div class="name">Lounge</div>
									</div>
								</div>
				...
				setTimeout(function () {
					document.getElementById('topic-groupList-fwd-' + elem.parent).getElementsByClassName('card-body')[0].insertAdjacentHTML('beforeend', htmlContent);
				}, 100);
			}
			...
			if (isTopicExist.length > 0) {
				isTopicExist
					...
					.forEach((elem, index) => {
						...
						setTimeout(function () {
							document.getElementById('topic-groupList-fwd-' + elem.group_id).getElementsByClassName('card-body')[0].insertAdjacentHTML('beforeend', htmlContent);
						}, 100);
					});
			}
		});
...
let addMessageToMessageArea = (msg) => {
	...
	if (chat.isGroup) {
		...
		<div style="cursor: pointer;" onclick="contactInfo('${groupMembers.find(el => el.f_pin === msg.sender).f_pin}');" class="small font-weight-bold text-primary group-username">
			${msg.sender == user.id ? user.name : groupMembers.find(el => el.f_pin === msg.sender).name}
		</div>
		`;
	}
	...
	//var root = 'http://202.158.33.27:2809';
	...
	if (urlRegex.test(msgText)) { // contains url
			...
			if (msg.is_complain == 1) {
				...
				<div class="align-self-${msg.sender === user.id ? "end self" : "start"} p-1 my-1 mx-3 rounded bg-white shadow-sm message-item" id="msg-${msg.id.toString()}">
					...
					<div class="d-flex flex-row">
						<div class="body m-1 mr-2">${msgBody != "" ? msgBody + "<br>" : ""}${msgText}</div>
...
let generateCallCenterEditor = () => {
			...
			<div class="body m-1 mr-2">
				...
				<div class="row">
					<div class="col-6">
						...
						<div class="cc-button py-2 px-4 m-2 btn-warning justify-content-center text-center align-self-center" id="start-ac-cc" style="cursor:pointer;">
					...
					<div class="col-6">
						...
						<div class="cc-button py-2 px-4 m-2 btn-warning justify-content-center text-center align-self-center" id="start-vc-cc" style="cursor:pointer;">
	...
	<div class="align-self-start p-1 my-1 mx-3 rounded bg-white shadow-sm message-item">
		<div class="d-flex flex-row">
			<div class="body m-1 mr-2">
				<i>Sorry, currently all our representatives are busy helping other customers. Please try again later, and thank you for your patience!</i>
			</div>
			<div class="time ml-auto small text-right flex-shrink-0 align-self-end text-muted" style="width:75px;">
				${mDate().getTime()}
			</div>
	...
	document.getElementById('start-vc-cc').addEventListener('click', () => {
		...
		xmlHttp.onreadystatechange = function () {
			...
			if (xmlHttp.readyState == 4 && xmlHttp.status == 200) {
				...
				let delay = new Date().getTime() - time;
...
let generateMessageArea = (elem, chatIndex, isNewChat, isGroup) => {
	...
	if (chat.is_complain == 1 || (chat.isGroup == true && chat.group.hasOwnProperty("is_organization") && chat.group.is_organization == 1) || (chat.isGroup && chat.group.name == "Internal" && user.user_type == 0) || (!chat.isGroup && chat.contact.id == "-999")) {
		// console.log('is true');
		DOM.messageAreaPic.removeAttribute("onclick");
	} else {
		DOM.messageAreaPic.setAttribute('onclick', 'showInfo()');
...
let showComplaintHistory = () => {
	...
	complaint_history
		...
		<div class="chat-list-item d-flex flex-row w-100 p-2 border-bottom" onclick="generateComplaintHistoryMessages(this, '${elem.id}')">
			<img src="/chatcore/assets/img/document.png" alt="Profile Photo" class="img-fluid rounded-circle mr-2" style="height:50px; width:50px;">
			<div class="w-50">
				...
				<div id="accordion-groupList-${elem.id}" style="width:100%;">
					<div class="card">
						<div class="card-header" id="groupList-${elem.id}">
							<div class="chat-list-item d-flex flex-row w-100 p-2 border-bottom collapsed" data-toggle="collapse" data-target="#topic-groupList-${elem.id}" aria-expanded="false" aria-controls="topic-groupList-${elem.id}">
								<img src="${elem.pic}" alt="Profile Photo" class="img-fluid rounded-circle mr-2" style="height:50px; width:50px;">
								<div class="w-50 align-self-center">
									<div class="name">${elem.name}</div>
				...
				.forEach((elem, index) => {
					if (elem.group_id == chat.group.id) {
						...
						<div id="groupMember-${elem.f_pin}" class="chat-list-item d-flex flex-row w-100 p-2 border-bottom" ${elem.f_pin != user.id ? "onclick=\"contactInfo('" + elem.f_pin + "')\"" : ""}>
							<img src="${elem.f_pin == user.id ? user.pic : elem.pic}" alt="Profile Photo" class="img-fluid rounded-circle mr-2 member-pp" style="height:50px; width:50px;">
							<div class="w-50">
...
let checkOngoingComplaint = () => {
	if (chat == null) {
		if (ongoingComplaint && Object.keys(ongoingComplaint).length > 0) {
			...
			if (user.user_type != 24) { // if not staff
...
let init = () => {
	...
	} else if (user.id == localStorage.cc_alias_fpin) { // is company account
		DOM.personLabel.innerHTML = '<i class="fas fa-check-circle" style="color:#306EFF;"></i> Official';
	}
...
DOM.broadcastFile.addEventListener('change', (e) => {
	if (DOM.broadcastFile.files[0].size > 250000000) {
		alert("File size is limited to 250MB");
...
let broadcastCreateVC = () => {
	...
	let title = DOM.broadcastTitle.value;
	let start = combineStartTime();
	let members = selectedParticipant.join();
