...
let fillGroupDataList = () => {
	...
	$("#target-group-list").autocomplete({
		...
		select: function (event, ui) {
			let grp_id = groupIsMember.find((el) => el.name.trim() === ui.item.value.trim()).id;
...
let combineStartTime = () => {
	...
	if (currentTime - startDateTimeMillis > 60000) {
		...
		return startDateTimeMillis + 60000;
	} else {
...
let sendBroadcast = () => {
	...
	if (startDateFlag && endDateFlag && selectedParticipantFlag && selectedGroupFlag && titleFlag && messageFlag) {
		...
		alert("Please complete and check your broadcast form first");
		disableBroadcast = false;
	}
};
...
function openGroupSelect() {
	let f_pin = localStorage.F_PIN;

	$("#select-target-group .modal-body").html("")
	...
	xmlHttp.onreadystatechange = function () {
		if (xmlHttp.readyState == 4 && xmlHttp.status == 200) {
			...
			let html = '<ul class="group-hierarchy" style="list-style:none;">';

			data.forEach(grp => {
				html += `
				<li id="group-${grp.GROUP_ID}">
					<img class="${grp.CHILD_AMT > 0 ? "" : "d-none"}" id="chevron-${grp.GROUP_ID}" src="/chatcore/assets/img/chevron.png" style="width: 15px; height: 15px" onclick="expandGroup('${grp.GROUP_ID}');">
					${grp.GROUP_NAME}
				</li>
				`;
			})
			...
			$("#select-target-group .modal-body").html(html);
			$("#select-target-group").modal("show");
		}
	}
	xmlHttp.open("get", "/chatcore/logics/digisales/fetch_own_group?f_pin=" + f_pin);
	xmlHttp.send();
	// }
}
...
function expandGroup(groupId) {
	...
	xmlHttp.onreadystatechange = function () {
		if (xmlHttp.readyState == 4 && xmlHttp.status == 200) {
			...
			let selectedGroup = document.querySelector(".group-hierarchy li#group-" + groupId);

			selectedGroup.innerHTML += html;
		}
	}
	xmlHttp.open("get", "/chatcore/logics/fetch_subgroup?p_id=" + groupId);
	xmlHttp.send();
}
