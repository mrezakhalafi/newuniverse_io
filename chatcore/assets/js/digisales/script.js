...
let viewChatList = (list) => {
	...
	if (user.be !== 308) {
		if (chatList.length > 0) {
				...
				.forEach((elem, index) => {
					...
					if (!elem.isGroup) {
						...
						if (elem.is_complain == 1) {
							DOM.chatList.innerHTML += `
							<div id="${elem.contact.id}" class="chat-list-item d-none flex-row w-100 p-4 bg-lightgray border-bottom ${unreadClass} d-none" onclick="generateMessageArea(this, ${index}, false, false)">
								<img src="${elem.isGroup ? elem.group.pic : elem.contact.pic}" onerror="this.src='/chatcore/assets/img/grey_circle.png'" alt="Profile Photo" class="img-fluid rounded-circle mr-2" style="height:50px; width:50px;">
					...
					elem.msg.sender === user.id ? statusClass : ""
							...
							if (elem.msg.hasOwnProperty("title")) {
								DOM.chatList.innerHTML += `
								<div class="chat-list-item d-flex flex-row w-100 p-4 bg-lightgray border-bottom ${unreadClass} d-none" onclick="generateMessageArea(this, ${index}, false, false)">
									<img src="${elem.isGroup ? elem.group.pic : elem.contact.pic}" onerror="this.src='/chatcore/assets/img/grey_circle.png'" alt="Profile Photo" class="img-fluid rounded-circle mr-2" style="height:50px; width:50px;">
									<div class="w-50 name-last-message">
										<div class="name">${badge} ${elem.name}</div>
			...
			DOM.chatList.innerHTML = `
			<div class="chat-list-item d-flex flex-row w-100 p-4">
				<span class="mx-auto mt-3">There are no conversations</span>
			</div>
			`;
		}
	} else {
			...
			groupList
				...
				.forEach((elem, index) => {
					if (elem.parent === "") {
							...
							<div class="card" style="background-color: #f7f7f7">
								<div class="card-header" id="groupList-${elem.id}">
									<div class="chat-list-item d-flex flex-row w-100 p-4 bg-lightgray border-bottom collapsed" data-toggle="collapse" data-target="#topic-groupList-${elem.id}" aria-expanded="false" aria-controls="topic-groupList-${elem.id}">
										<img src="${elem.pic}" onerror="this.src='/chatcore/assets/img/grey_circle.png'" alt="Profile Photo" class="img-fluid rounded-circle mr-2" style="height:50px; width:50px;">
										<div class="w-50 name-last-message">
											<div class="name">${elem.name}</div>
								...
								<div id="topic-groupList-${elem.id}" class="collapse" aria-labelledby="groupList-${elem.id}" data-parent="#accordion-groupList-${elem.id}">
									<div class="card-body">
										<div class="chat-list-item d-flex flex-row w-100 p-4 bg-lightgray border-bottom">
											<img src="/chatcore/assets/img/forum.png" onerror="this.src='/chatcore/assets/img/grey_circle.png'" alt="Profile Photo" class="img-fluid rounded-circle mr-2" style="height:50px; width:50px;">
											<div class="w-50 align-self-center">
												<div class="name">Lounge</div>
											</div>
										</div>
						...
						`;
						// onclick="generateMessageArea(this, '${elem.id}', true, true)"
					} else {
						// // console.log(elem);
						let htmlContent = `
						<div id="accordion-groupList-${elem.id}" style="width:100%;">
					...
					if (isTopicExist.length > 0) {
						isTopicExist
							...
							.forEach((elem, index) => {
								//  onclick="generateMessageArea(this, '${elem.id}', true, true)
								let htmlContent = `<div class="chat-list-item d-flex flex-row w-100 p-2 border-bottom"">
									<img src="${elem.pic}" onerror="this.src='/chatcore/assets/img/grey_circle.png'" alt="Profile Photo" class="img-fluid rounded-circle mr-2" style="height:50px; width:50px;">
									<div class="w-50  name-last-message">
	...
	if (msg.reply_to != null && msg.is_deleted == 0) {
		...
		if (replied_msg != undefined && replied_msg.is_deleted != 1) {
			...
			if (replied_msg.body == "") {
				...
				<div class="d-flex flex-row" style="background-color:${msg.sender == user.id ? "lightgray" : "#ff00008f"};">
...
let generateMessageArea = (elem, chatIndex, isNewChat, isGroup) => {
	...
	mClassList(DOM.openAttach).add("d-block");
	mClassList(DOM.openAttach).remove("d-none");
	mClassList(DOM.messageAreaPic).remove("d-none");
		...
		if (DOM.searchFriend.value.trim() == "") {
			...
			if (isFriendListOpen) {
				...
				<div class="chat-list-item d-flex flex-row w-100 p-2 border-bottom" ${elem.id === localStorage.cc_alias_fpin ? 'id="alias-' + elem.id + '"' : ""}  onclick="generateMessageArea(this, '${elem.id}', true, false)">
					<img src="${elem.pic}" onerror="this.src='/chatcore/assets/img/grey_circle.png'" alt="Profile Photo" class="img-fluid rounded-circle mr-2" style="height:50px; width:50px; object-fit:cover;">
					<div class="w-50">
						<div class="name">${badge} ${elem.name}</div>
					</div>
				</div>
				...
				let matchingFriends = groupList.filter((elem) => elem.name.toLowerCase().includes(DOM.searchFriend.value));
				console.log(matchingFriends);
					...
					.forEach((elem, index) => {
						if (elem.parent === "") {
							...
							setTimeout(function () {
								...
								// 	.getElementsByClassName("card-body")[0]
								// 	.insertAdjacentHTML("beforeend", htmlContent);
								DOM.groupList.innerHTML += htmlContent;
							}, 100);
						}
				...
				let matchingChatList = chatList.filter((elem) =>
					elem.name.includes(DOM.searchFriend.value) ||
					(elem.msg.body && elem.msg.body.includes(DOM.searchFriend.value)) ||
					(elem.msg.title && elem.msg.title.includes(DOM.searchFriend.value)) ||
					(elem.msg.description && elem.msg.description.includes(DOM.searchFriend.value))
				);

				console.log(matchingChatList);

				viewChatList(matchingChatList);
			}
		}
	}
});
...
let hideFriendList = () => {
	...
	// DOM.chatListNavbar.innerHTML =
	// 	`
	// <img onerror="this.src='/chatcore/assets/img/grey_circle.png'" alt="Profile Photo" class="img-fluid rounded-circle mr-2" style="height:50px; width:50px; cursor:pointer;" onclick="showProfileSettings()" id="display-pic" src="` +
	// 	user.pic +
	// 	`">
	// <div class="w-50">
	...
	DOM.chatListNavbar.innerHTML = `
		...
		<div class="nav-item dropdown ml-auto ${user.be === 308 ? "d-none" : ""}">
		...
		<div class="nav-item dropdown">
			...
			<div class="dropdown-menu dropdown-menu-right">
				<a class="dropdown-item ${user.be === 308 ? "d-none" : ""}" href="#" onclick="showGroupList()">View Groups</a>
				<a class="dropdown-item ${user.be === 308 ? "d-none" : ""}" href="#" data-toggle="modal" data-target="#create-group-modal">Create Group</a>
				...
				<a class="dropdown-item ${user.user_type == 0 ? "d-none" : ""}" href="#" id="broadcast-history" onclick="showBCHistory();">Broadcast History</a>
				<form method='POST'>
					<input type="submit" class="dropdown-item" name="submit" id="submit" value="Log Out">
				</form>
			</div>
...
let showComplaintHistory = () => {
	...
	complaint_history
		...
		<div class="chat-list-item d-flex flex-row w-100 p-2 border-bottom" onclick="generateComplaintHistoryMessages(this, '${elem.id}')">
			<img src="/chatcore/assets/img/document.png" onerror="this.src='/chatcore/assets/img/grey_circle.png'" alt="Profile Photo" class="img-fluid rounded-circle mr-2" style="height:50px; width:50px;">
			<div class="w-50 name-last-message">
...
let showGroupList = () => {
	...
	groupList.forEach((elem) => {
		// if (elem.id != "-999" && elem.id != user.id) {
		groupNames.push(elem.name.trim());
		// }
	});
	$("#search-friend").autocomplete({
		source: groupNames,
...
let showInfo = () => {
	...
	if (chat != null) {
		if (!chat.isGroup) {
			...
			console.log('buka info', chat);

			// check group creator
			if (chat.group && chat.group.created_by == localStorage.F_PIN) {
				...
				.forEach((elem, index) => {
					if (elem.group_id == chat.group.id) {
						...
						<div id="groupMember-${elem.f_pin}" class="chat-list-item d-flex flex-row w-100 p-2 border-bottom" ${elem.f_pin != user.id ? "onclick=\"contactInfo('" + elem.f_pin + "')\"" : ""}>
							<img src="${elem.f_pin == user.id ? user.pic : elem.pic}" onerror="this.src='/chatcore/assets/img/grey_circle.png'" alt="Profile Photo" class="img-fluid rounded-circle mr-2 member-pp" style="height:50px; width:50px;">
							<div class="w-50 name-last-message">
...
function fetchOutletCategory() {
	var xmlHttp = new XMLHttpRequest();
	xmlHttp.onreadystatechange = function () {
		if (xmlHttp.readyState == 4 && xmlHttp.status == 200) {
			// get friends lpins
			console.log("OUTLET", JSON.parse(xmlHttp.responseText));
			outletCategory = JSON.parse(xmlHttp.responseText);

			let html = ``;
			outletCategory.forEach(out => {
				html += `
				<label for="user-${out.NAME}" class="mt-1">
					<input type="radio" value="${out.ID}" name="user-category" id="user-${out.NAME}" class="user-category">
					${out.NAME}
				</label>`;
			})

			$("#user-category-list").append(html);
		}
	};
	xmlHttp.open("get", "/chatcore/logics/digisales/fetch_outlet_category?f_pin=" + user.id);
	xmlHttp.send();
}
...
let init = () => {
	...
	if (executed === false) {
		if (user.user_type == 0) {
			...
			document.querySelector("#broadcast-history").classList.add("d-none");
		}
		// appendUserType();
		// fetchReadMessages();
	...
	groupList
		...
		.forEach((elem, index) => {
			if (elem.name !== "Customer Service" || elem.name !== "Internal") {
				...
				groupListOption.push(elem.name.trim());
			}
		});
	$("#target-group-list").autocomplete({
		source: groupListOption,
		minLength: 3,
		select: function (event, ui) {
			let grp = groupList.find((el) => el.name.trim() === ui.item.value.trim());
			let grp_lv = grp.level;
			let grp_id = grp.id;
			console.log('pick group', grp_id);
			selectedGroup.push(grp_id);
			tempGroup.push({
				id: grp_id,
				level: grp_lv
			});

			// HIDE CATEGORY IF SELECT OTHER THAN LV 7
			let notLv6 = tempGroup.filter((el) => el.level != 7);
			let isLv6 = tempGroup.filter((el) => el.level == 7);
			console.log(notLv6);

			if (notLv6.length > 0 && isLv6.length > 0) { // MIXED -> HIDE BOTH
				console.log("MIXED")
				$("div#also-children").addClass("d-none")
				$("input#broadcast-to-children").prop("checked", false)
				$("#user-category-list").addClass("d-none");
				$("input[type=radio].user-category").prop("checked", false);
			} else if (notLv6.length > 0 && isLv6.length == 0) { // BUKAN KABUPATEN -> HIDE CATEGORY, SHOW ALSO...
				console.log("NOT KAB")
				$("div#also-children").removeClass("d-none")
				$("#user-category-list").addClass("d-none");
				$("input[type=radio].user-category").prop("checked", false);
			} else if (notLv6.length == 0 && isLv6.length > 0) { // KABUPATEN -> SHOW CATEGORY, HIDE ALSO...
				console.log("ONLY KAB")
				$("#user-category-list").removeClass("d-none")
				$("div#also-children").addClass("d-none")
				$("input#broadcast-to-children").prop("checked", false)
			}
			...
			DOM.broadcastGroup.value += ui.item.value.trim() + ", ";
			// DOM.participantListDropdown.value = "";
		},
		appendTo: "#form-target-group",
	});
};

// let userCategoryCheckbox = document.querySelector(".user-category");
...
function testBCDate(testDate = "", testTime = "") {
...
DOM.broadcastStartDate.addEventListener("change", (e) => {
	...
	// if (testBCDate(startdatepart, starttimepart) == false) {
	// 	alert("Please input today's or future date.")
	// } else {
	combineStartTime();
	// }
});
...
DOM.broadcastEndDate.addEventListener("change", (e) => {
	...
	// if (testBCDate(enddatepart, endtimepart) == false) {
	// 	alert("Please input today's or future date.")
	// } else {
	combineEndTime();
	// }
...
let check_subgroup = false;
document.querySelector("#broadcast-to-children").addEventListener("change", function () {
	if (this.checked) {
		check_subgroup = true;
	} else {
		check_subgroup = false;
...
let sendBroadcast = () => {
	...
	if (check_subgroup == true && target == "4") {
		target = "7";
	}
	...
	if (DOM.broadcastFile.files.length !== 0) {
		...
		} else if (!isImage(getExtension(file.name)) && !isVideo(getExtension(file.name))) {
			category = "3"; // document
			hex = Date.now().toString(16);
			formData.append("file", file);
			formData.append("hex", hex);
	...
	if (target === "4" || target === "7") {
		if (selectedGroup.length > 0) {
			...
			if ($(".user-category:checked").val() !== undefined) {
				formData.append("user_category", parseInt($(".user-category:checked").val()));
			}
		} else {
	...
	xmlHttp.onreadystatechange = function () {
		if (xmlHttp.readyState == 4 && xmlHttp.status == 200) {
			...
			if (xmlHttp.responseText == "Success") {
				...
				$("#broadcast-modal-result #bc-success").removeClass("d-none");
				$("#broadcast-modal-result #bc-error").addClass("d-none");
				// $("#broadcast-modal-result").modal("show");
				DOM.broadcastForm.reset();
				selectedGroup = [];
			} else {
				$("#broadcast-modal-result #bc-success").addClass("d-none");
				$("#broadcast-modal-result #bc-error").removeClass("d-none");
				// $("#broadcast-modal-result").modal("show");
	...
	xmlHttp.upload.addEventListener('progress', function (e) {
		if (e.lengthComputable && DOM.broadcastFile.files.length !== 0) {
			$("#broadcast-modal-result").modal("show");
			$("#broadcast-modal-result #bc-progress").removeClass("d-none");

			var uploadPercent = e.loaded / e.total;
			uploadPercent = Math.round(uploadPercent * 100);
			console.log("PROGRESS:", uploadPercent);

			// $("#broadcast-modal-result #bc-progress").text("Progress : " + uploadPercent + "%")
			var elem = document.getElementById("barStatus");
			elem.style.width = uploadPercent + '%';
	...
	if (startDateFlag && endDateFlag && selectedParticipantFlag && selectedGroupFlag && titleFlag && messageFlag) {
		...
		alert("Please check your broadcast form for incomplete or invalid inputs, and broadcast start time can not be longer than one minute ago.");
		disableBroadcast = false;
	}
};
...
function submitNewSubgroup() {
	...
	xhr.open("POST", "/chatcore/logics/submit_create_subgroup", false);
	xhr.send(formData);
}

let bcHistoryOpen = false;

function showBCHistory() {
	bcHistoryOpen = true;

	mClassList(DOM.messageAreaOverlay).add("d-none");
	mClassList(DOM.openAttach).remove("d-block").add("d-none");
	mClassList(DOM.messageArea).remove("d-none").add("d-flex");
	mClassList(DOM.messageAreaPic).add("d-none");

	DOM.messageAreaName.innerHTML = "Broadcast History";
	mClassList(DOM.finishComplain).add("d-none");
	mClassList(DOM.createTopicButton).add("d-none");
	mClassList(DOM.deleteConversation).add("d-none");
	...
	console.log(bcHistory);
	bcHistory.forEach(m => {
		addBCtoBCArea(m);
	})
}	

function addBCtoBCArea(msg) {
	var root = location.protocol + '//' + location.host;
	let attach = "";
	if (msg.image != null) {
		attach = `<img src="${root + dir + msg.image}" style="width:250px; height:auto; cursor:pointer;" onclick="openImageNewTab('${root + dir + msg.image}')">`;
	} else if (msg.video != null) {
		attach = `
				<video controls width="250">
					<source src='${root + dir + msg.video}'>
					Sorry, your browser doesn't support embedded videos.
				</video>
			`;
	} else if (msg.audio != null) {
		attach = '<div class="flex-grow-1"><img src="../../assets/img/thumb_audio.png" width="100" height="100"><span><a href="' + root + dir + msg.audio + '" target="_blank">' + msg.audio + "</a></span></div>";
	} else if (msg.file != null) {
		attach = '<div class="flex-grow-1"><img src="../../assets/img/thumb_document.png" width="100" height="100"><span><a href="' + root + dir + msg.file + '" target="_blank">' + msg.file + "</a></span></div>";
	}

	let urlRegex = new RegExp("([a-zA-Z0-9]+://)?([a-zA-Z0-9_]+:[a-zA-Z0-9_]+@)?([a-zA-Z0-9.-]+\\.[A-Za-z]{2,4})(:[0-9]+)?([^ ])+");
	let finalUrl = "";
	if (urlRegex.test(msg.link)) {
		// contains url

		let matchingRegex = msg.link.match(urlRegex)[0];
		let correctedRegex = matchingRegex;
		if (!matchingRegex.includes("https://") && !matchingRegex.includes("http://")) {
			correctedRegex = "https://" + matchingRegex;
		} else if (matchingRegex.includes("http://")) {
		...
		finalUrl = "<a href=" + correctedRegex + ' target="_blank">' + matchingRegex + "</a>";
	}

	let body = `
		${richText(msg.title)}
		<br><br>
		${richText(msg.content)}		
		${finalUrl != "" ? "<br><br>" + finalUrl + "<br>" : ""}
		${attach != "" ? "<br>" + attach : ""}
	`;

	let html = `
	<div class="align-self-end self my-1 mx-3 rounded bg-white message-item" id="msg-${msg.id}">
		<div class="options dropdown">
		...
		<div class="d-flex flex-row px-3 py-3 bg-purple" style="border-radius: 10px;">
			<div class="body" style="font-size: 14px;">
				${body}
			</div>
		</div>
		<div class="time ml-auto text-right flex-shrink-0 align-self-end text-muted" style="width:75px; font-size: 10px;">
			${mDate(msg.start).getTime()}
			<i class="fas fa-check-double d-none" style="color: #6960EC;"></i>
		</div>
	</div>
	`;
