...
if (!isset($_POST['generate-apk']) && !isset($_POST['edit_apk'])) {
    ...
    if (isset($_POST['check-certif'])) {
        if ($company_logo != null) {
            ...
            if (isset($_POST['tab5']) && trim($_POST['tab5']) !== "") {
                $tab5 = $_POST['tab5'];
                $sql .= ", TAB5";
            }
            if (isset($_POST['tab6']) && trim($_POST['tab6']) !== "") {
                $tab6 = $_POST['tab6'];
                $sql .= ", TAB6";
            }
            ...
            if (isset($_FILES['tab5_icon']) && $_FILES['tab5_icon']['size'] != 0) {
                // No file was selected for upload, your (re)action goes here
                if (move_uploaded_file($_FILES['tab5_icon']['tmp_name'], $_SERVER['DOCUMENT_ROOT'] . '/dashboardv2/uploads/tab_icon/' . $id_company . '_' . $_FILES['tab5_icon']['name'])) {
                    $sql .= ", TAB5_ICON";
                    $tab5_icon = $id_company . '_' . $_FILES['tab5_icon']['name'];
                }
            }

            if (isset($_FILES['tab6_icon']) && $_FILES['tab6_icon']['size'] != 0) {
                // No file was selected for upload, your (re)action goes here
                if (move_uploaded_file($_FILES['tab6_icon']['tmp_name'], $_SERVER['DOCUMENT_ROOT'] . '/dashboardv2/uploads/tab_icon/' . $id_company . '_' . $_FILES['tab6_icon']['name'])) {
                    $sql .= ", TAB6_ICON";
                    $tab6_icon = $id_company . '_' . $_FILES['tab6_icon']['name'];
                }
            }
            ...
            // FB CONTENT
            if (isset($_POST['custom_buttons'])) {
                $custom_buttons = $_POST['custom_buttons'];
                $sql .= ", CUSTOM_BUTTON";
            }
            ...
            if (isset($_FILES['app_builder_button_icon'])) {
                $countFBIcon = count($_FILES['app_builder_button_icon']['name']);
                if ($countFBIcon > 0) {
                    $folderUpload = $_SERVER['DOCUMENT_ROOT'] . '/dashboardv2/uploads/fb_icon';
                    for ($i = 0; $i < $countFBIcon; $i++) {
                        $namaFile = $_FILES['app_builder_button_icon']['name'][$i];
                        $lokasiTmp = $_FILES['app_builder_button_icon']['tmp_name'][$i];
    
                        # kita tambahkan uniqid() agar nama gambar bersifat unik
                        $namaBaru = $id_company . '_' . $namaFile;
                        $lokasiBaru = "{$folderUpload}/{$namaBaru}";
                        if (move_uploaded_file($lokasiTmp, $lokasiBaru)) {
                            array_push($button_names, $namaBaru);
                        }
                    }
                    $custom_button_icon = implode(",",$button_names);
                    $sql .= ", CUSTOM_BUTTON_ICON";
                }
            }
            // END FB ICON

            // FB URL
            if (isset($_POST['app_builder_button_url'])) {
                $custom_button_url = $_POST['app_builder_button_url'];
                $sql .= ", CUSTOM_BUTTON_URL";
            }
            // END FB URL
            ...
            if (isset($_FILES['background'])) {
                ...
                if ($jumlahFile > 0) {
                    ...
                    for ($i = 0; $i < $jumlahFile; $i++) {
                        ...
                        $lokasiBaru = "{$folderUpload}/{$namaBaru}";
                        if (move_uploaded_file($lokasiTmp, $lokasiBaru)) {
                            array_push($bg_names, $namaBaru);
                        }
                    }
                    $background = implode(",", $bg_names); 
                    $list_bg = $background;
                    $sql .= ", APP_BG";
                }
            ...
            if (isset($_POST['tab5']) && trim($_POST['tab5']) !== "") {
                $sql .= ", '$tab5'";
                array_push($tab_arr, $tab5);
            }
            if (isset($_POST['tab6']) && trim($_POST['tab6']) !== "") {
                $sql .= ", '$tab6'";
                array_push($tab_arr, $tab6);
            }
            ...
            if ($query->execute()) {
                ...
                // FB/DOCKED VALUES
                if ($custom_buttons != "") {
                    $sqlPrefs = "REPLACE INTO `PREFS` (`BE`, `KEY`, `VALUE`) VALUES ($be_id, 'app_builder_custom_buttons', '$custom_buttons')";
                    $prefsBg = $dbConnPalio->prepare($sqlPrefs);
                    $prefsBg->execute();
                    $prefsBg->close();
                }
                if ($custom_button_url != "") {
                    $sqlPrefs = "REPLACE INTO `PREFS` (`BE`, `KEY`, `VALUE`) VALUES ($be_id, 'app_builder_button_url', '$custom_button_url')";
                    $prefsBg = $dbConnPalio->prepare($sqlPrefs);
                    $prefsBg->execute();
                    $prefsBg->close();
                }
                if ($custom_button_icon != "") {
                    $sqlPrefs = "REPLACE INTO `PREFS` (`BE`, `KEY`, `VALUE`) VALUES ($be_id, 'app_builder_button_icon', '$custom_button_icon')";
                    $prefsBg = $dbConnPalio->prepare($sqlPrefs);
                    $prefsBg->execute();
                    $prefsBg->close();
                }
                ...
                if ($gps_pushkit == 1) {
                    ...
                    $sqlPrefs = "REPLACE INTO `SERVICE_ACCOUNTS` (`BUSINESS_ENTITY`, `TYPE`) VALUES ($be_id, 1)";
                    $prefsBg = $dbConnPalio->prepare($sqlPrefs);
                    $prefsBg->execute();
                    $prefsBg->close();
                }
                
                $news_tl = $_POST["news_tl"];
                if ($app_url == "2" || $url_content == "2") {
                    
                    $sqlPrefs = "REPLACE INTO `POST_TRANSLATION_SETTING` (`BUSINESS_ENTITY`, `ENABLE_TL`, `ENABLE_NEWS`) VALUES ($be_id, '$news_tl', 1)";
                    $prefsBg = $dbConnPalio->prepare($sqlPrefs);
                    $prefsBg->execute();
                    $prefsBg->close();
                } else {
                    $sqlPrefs = "REPLACE INTO `POST_TRANSLATION_SETTING` (`BUSINESS_ENTITY`, `ENABLE_TL`, `ENABLE_NEWS`) VALUES ($be_id, '0', 0)";
                    $prefsBg = $dbConnPalio->prepare($sqlPrefs);
                    $prefsBg->execute();
                    $prefsBg->close();
                }

                // KALO MAU NGETES COMMENT DUA BARIS CODING DIBAWAH INI
    ...
    if (isset($_POST['tab4_edit']) && trim($_POST['tab4_edit']) !== "") {
        array_push($tab_arr, $_POST['tab4_edit']);
    }
    if (isset($_POST['tab5_edit']) && trim($_POST['tab5_edit']) !== "") {
        array_push($tab_arr, $_POST['tab5_edit']);
    }
    if (isset($_POST['tab6_edit']) && trim($_POST['tab6_edit']) !== "") {
        array_push($tab_arr, $_POST['tab6_edit']);
    }
    ...
    if (isset($_FILES['tab5_icon_edit']) && $_FILES['tab5_icon_edit']['size'] != 0) {

        if (move_uploaded_file($_FILES['tab5_icon_edit']['tmp_name'], $_SERVER['DOCUMENT_ROOT'] . '/dashboardv2/uploads/tab_icon/' . $id_company . '_' . $user_access . '_' . $_FILES['tab5_icon_edit']['name'])) {
            $filename = $id_company . '_' . $user_access . '_' . $_FILES['tab5_icon_edit']['name'];
            
            $query = "($be_id, 'app_builder_tab5_icon', '$filename', $user_access)";
            array_push($arr_query, $query);
        }

    }

    if (isset($_FILES['tab6_icon_edit']) && $_FILES['tab6_icon_edit']['size'] != 0) {

        if (move_uploaded_file($_FILES['tab6_icon_edit']['tmp_name'], $_SERVER['DOCUMENT_ROOT'] . '/dashboardv2/uploads/tab_icon/' . $id_company . '_' . $user_access . '_' . $_FILES['tab6_icon_edit']['name'])) {
            $filename = $id_company . '_' . $user_access . '_' . $_FILES['tab6_icon_edit']['name'];
            
            $query = "($be_id, 'app_builder_tab6_icon', '$filename', $user_access)";
            array_push($arr_query, $query);
        }
