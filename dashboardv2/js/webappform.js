...
// HUAWEI JSON VALIDATION
function onChange(event) {
    var reader = new FileReader();
    reader.onload = validatePackageName;
...
function validatePackageName(event){
    ...
    console.log("HUAWEI", obj);

    let appId = document.querySelector("input#appId").value;
    
    let jsonPackageName1 = obj.appInfos[0].package_name;
    let jsonPackageName2 = obj.appInfos[0].app_info.package_name;
    let jsonPackageName3 = obj.app_info.package_name;

    let huawei_warning = document.querySelector("#huawei_pushkit_validation");

    if (appId != jsonPackageName1 || appId != jsonPackageName2 || appId != jsonPackageName3) {
...
document.getElementById('huawei_pushkit').addEventListener('change', onChange);
// END HUAWEI JSON VALIDATION
...
function sendData() {
    ...
    $('#build-start-time').html('Build start time = <b>' + hoursBuild + " : " + minutesBuild + '</b>');
    $('#build-end-time').html('Estimated end time = <b>' + hoursEnd + " : " + minutesEnd + '</b>');
