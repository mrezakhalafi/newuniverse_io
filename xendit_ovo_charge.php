...
<?php
    ...
    $post_transaction_id = $_POST["transaction_id"];
    // $amount = $_POST["amount"];

    // AMOUNT FOR TESTING ONLY
    $amount = 2500.00;
    // $amount = 0;
    ...
    function saveData(){
        ...
        global $post_company_id;
        global $post_transaction_id;

        $company_id = $post_company_id;
        $transaction_id = $post_transaction_id;
    ...
    function checkOvo(){
        ...
        $url_check = "https://api.xendit.co/ewallets?external_id=" . $external_id . "&ewallet_type=OVO";
        ...
        $stream_check = stream_context_create($options_check); 
        $result_check = file_get_contents($url_check, false, $stream_check);
        $json_result_check = json_decode($result_check);

        if($json_result_check->status == "COMPLETED"){
            saveData();
            echo $json_result_check;

            // echo $json_result_check->status;
        } else {
            checkOvo();
            // echo $json_result_check->status;
        }
